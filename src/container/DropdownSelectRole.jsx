...
const DropdownSelectRole = ({
  ...
  const { roleData, companyId } = userGloabalContext();
  const [visible, setVisible] = useState(false);
  const [field, setField] = useState("");

  const RoleHandleDelete = async (id) => {
    try {
      wyraiApi
        .delete(`/api/roleDelete/${id}/${companyId}`)
        .then((res) => fetchRole())
      ...
      <div className=" relative flex flex-col">
        ...
        {visible && (
          <div className=" absolute w-full bg-white top-[98%] shadow mt-2">
            ...
            <ul className="pl-2 h-[130px] overflow-y-auto">
              {roleData.map((item, index) => (
                ...
                >
                  <li
                    className="py-2 pl-2 w-full cursor-pointer font-medium capitalize hover:shadow-md hover:rounded-sm"
                    onClick={() => {
                  ...
                  <FaRegTrashAlt
                    ...
                    onClick={() => RoleHandleDelete(item._id)}
                  />
                </div>
              ))}
            </ul>
