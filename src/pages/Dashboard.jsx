				...
				<div className="grid grid-cols-[30%_68%] gap-[2%] w-full ">
						<div className='shadow-md bg-white p-2'>
							...
							{Object.keys(status).map((item,index) => (
									<div key={index} className="h-[130px] flex flex-col justify-around items-center" style={{color:`${status[item].color}`, borderBottom: `3px solid ${status[item].color}`}}>
										<span className='text-base font-medium text-black'>{status[item].name}</span>
